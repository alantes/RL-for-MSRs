...
class Environment(gym.Env):
    ...
    def step(self, action):
        ...
        #reward -= 0.000001 * np.sum(self.height_indicator)
        ...
        first_vec = self.shearable_rod.position_collection[...,-1] - self.shearable_rod.position_collection[...,32]
        second_vec = self.shearable_rod.position_collection[...,0] - self.shearable_rod.position_collection[...,32]
        mid_vec = (first_vec + second_vec)/2
        mid_angle = np.angle(mid_vec[0] + mid_vec[1] * 1j)

        self.last_rod_position = current_mid_position.copy()
        span = np.linalg.norm(self.shearable_rod.position_collection[...,-1] - self.shearable_rod.position_collection[...,0])
        return state, reward, done, {"ctime": self.time_tracker, "overtime": is_too_many_timesteps, "magnetic_field": self.external_magnetic_field.copy(), 
                                    "height": self.shearable_rod.position_collection[...,32][1], "span": span, 
                                    "forward_position": self.shearable_rod.position_collection[...,-1][0],
                                    "mid_position": self.shearable_rod.position_collection[...,32][0],
                                    "back_position": self.shearable_rod.position_collection[...,0][0],
                                    "forward_contact": self.ground_contanct_dir[-1],
                                    "back_contact": self.ground_contanct_dir[0],
                                    "mean_angle": mid_angle,
                                    "mean_angle_vel": 0}
